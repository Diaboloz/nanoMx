 ...
 * @version $Id: Savant3_Plugin_image.php 6 2015-07-08 07:07:06Z PragmaMx $
 */
...
class Savant3_Plugin_image extends Savant3_Plugin {
        ...
        $attr = null)
    {
        ...
        if (stristr($_SERVER['HTTP_USER_AGENT'], 'MSIE') && $type === 3) {
            ...
            $html .= "(src='" . htmlspecialchars($file, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . "',sizingMethod='scale');\"";
            $html .= ' title="' . htmlspecialchars($alt, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . '"';
            ...
            $html = '<img';
            $html .= ' src="' . htmlspecialchars($file, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . '"';
            // add the alt attribute
            if (is_null($alt)) {
                $alt = basename($file);
            }
            $html .= ' alt="' . htmlspecialchars($alt, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . '"';
            // add the height attribute
            if ($height) {
                $html .= ' height="' . htmlspecialchars($height, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . '"';
            }
            // add the width attribute
            if ($width) {
                $html .= ' width="' . htmlspecialchars($width, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . '"';
            }
