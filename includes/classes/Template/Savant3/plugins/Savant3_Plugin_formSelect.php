 ...
 * @version $Id: Savant3_Plugin_formSelect.php 6 2015-07-08 07:07:06Z PragmaMx $
 */
        ...
        if ($disable) {
            ...
            foreach ($options as $opt_value => $opt_label) {
                if (in_array($opt_value, $value)) {
                    ...
                    $opt .= htmlspecialchars($opt_label, ENT_COMPAT | ENT_HTML5, 'UTF-8', false);
                    // add to the list
                    $list[] = $opt;
                }
            }
            ...
            foreach ($options as $opt_value => $opt_label) {
                $opt = '<option';
                $opt .= ' value="' . htmlspecialchars($opt_value, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . '"';
                $opt .= ' label="' . htmlspecialchars($opt_label, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . '"';
                if (in_array($opt_value, $value)) {
                    $opt .= ' selected="selected" class="current"';
                }
                $opt .= '>' . htmlspecialchars($opt_label, ENT_COMPAT | ENT_HTML5, 'UTF-8', false) . "</option>";
                $list[] = $opt;
            }
