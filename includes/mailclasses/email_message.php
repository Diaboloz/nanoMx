 ...
 * email_message.php
 *
 * @(#) $Id: email_message.php 6 2015-07-08 07:07:06Z PragmaMx $
...
<class>
	...
	<version>@(#) $Id: email_message.php 6 2015-07-08 07:07:06Z PragmaMx $</version>
...
{metadocument}
	<variable>
		<name>mailer</name>
		<type>STRING</type>
		<value>http://www.phpclasses.org/mimemessage $Revision: 6 $</value>
		<documentation>
...
*/
    ...
    Function GenerateMessageID($sender)
    {
        ...
        $host = $this->Tokenize(" @");
        if (strlen($host) && $host[strlen($host)-1] == "-")
    ...
    Function EndSendingMessage()
    {
        ...
        if (strlen($has["to"]) == 0 && !IsSet($has["cc"]) && !IsSet($has["bcc"]))
        ...
        if (!IsSet($has["message-id"]) && $this->auto_message_id) {
            $sender = array();
            ...
            for($s = 0; $s < $ts; ++$s) {
                ...
                if (strlen($error) == 0 && count($senders))
                    break;
            }
            if (count($senders) == 0)
    ...
    Function GetPartHeaders(&$headers, $part)
    {
        ...
        if (IsSet($this->parts[$part]['Cache']) && $this->parts[$part]['Cache'])
        ...
        switch ($type) {
            ...
            case "message":
                if (IsSet($this->parts[$part]["FILENAME"])) {
                    ...
                    fclose($file);
                    if ((GetType($size) == "integer" && strlen($body) > $size) || (function_exists("get_magic_quotes_runtime") && get_magic_quotes_runtime()))
        ...
        for($wrapped = "", $line = 0;$line < count($lines);++$line) {
            if (strlen($text_line = $lines[$line])) {
                for(;strlen($text_line = $line_prefix . $text_line) > $line_length;) {
                    if (GetType($cut = strrpos(substr($text_line, 0, $line_length), " ")) != "integer" || $cut < strlen($line_prefix)) {
...
*/
    Function CreatePlainTextPart($text, $charset, &$part)
    {
        ...
        $definition = array("Content-Type" => "text/plain",
            "DATA" => $text
            );
...
*/
    Function CreateHTMLPart($html, $charset, &$part)
    {
        if (!strcmp($charset, ""))
            $charset = $this->default_charset;
        $definition = array("Content-Type" => "text/html",
            "CHARSET" => $charset,
            "DATA" => $html
            );
        return($this->CreatePart($definition, $part));
...
*/
    ...
    Function GetFileDefinition($file, &$definition, $require_name = 1)
    {
        ...
        if (IsSet($file["Content-Type"])) {
            ...
            $content_type = "application/octet-stream";
        $definition = array("Content-Type" => $content_type,
        ...
        if (IsSet($file['Cache']) && $file['Cache'])
            $definition['Cache'] = 1;
        return("");
    }
...
*/
    ...
    {
        $definition = array("Content-Type" => "multipart/" . $type,
            "PARTS" => $parts
            );
        return($this->CreatePart($definition, $part));
    }
...
*/
    ...
    Function OpenMailing(&$mailing, &$mailing_properties)
    {
        ...
        $this->mailings[$mailing] = array("Envelope" => $envelope,
            "BasePath" => $base_path
            );
        return("");
    }
