...
*/
    ...
    {
        if (IsSet($this->smtp)) {
            if (!$this->smtp->Disconnect() && strlen($error) == 0)
    ...
    {
        if (function_exists("class_exists") && !class_exists("smtp_class"))
    ...
    Function SendMessageHeaders($headers)
    {
        ...
        if (!$this->smtp->StartData() || !$this->smtp->SendData($header_data . "\r\n"))
            return($this->ResetConnection($this->smtp->error));
        return("");
    }
    ...
    {
        ++$this->delivery;
        if ($this->bulk_mail && !$this->smtp_direct_delivery && ($this->maximum_bulk_deliveries == 0 || $this->delivery < $this->maximum_bulk_deliveries))
            return("");
