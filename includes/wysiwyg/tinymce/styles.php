...
foreach ($styles as $key => $value) {
    /* Überschrift: {title : 'the title'} */
    $parts[] = "{title : '" . $value['caption'] . "'}";
    foreach ($value['items'] as $item) {
        /* normaler style: {title : 'the title', selector : 'tr', classes : 'class'} */
        list($selector, $class) = explode('.', $item);
        switch ($value['type']) {
            case 'inline':
            case 'block':
                $parts[] = "{title:'" . $class . "'," . $value['type'] . ":'" . $selector . "',classes:'" . $class . "'}";
                break;
            default:
                $parts[] = "{title:'" . $item . "'," . $value['type'] . ":'" . $selector . "',classes:'" . $class . "'}";
        }
    }
}
array_shift($parts);
